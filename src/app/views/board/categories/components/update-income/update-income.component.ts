...
export class UpdateIncomeComponent{
    @Input() income!: IncomeResult;
    @Input() categoryId!:number;

    @Output() updated = new EventEmitter<IncomeResult>();
    ...
    fillForm = () => this.form.reset(this.income);
    ...
    save() {
        ...
        if (this.form.valid) {
            ...
            command.incomeId = this.income.incomeId;
            command.categoryId = this.categoryId;
            command.period = +this.f.period.value;
            
            this.spinner = true;
            this.service.updateIncome(command)
